...
type sclient struct {
	...
	// sent packets to this client. It is nil until the first packet.
	// Written by the writer goroutine ([sclient.sendPacket]), guarded by
	// senderCardinalityMu for [sclient.EstimatedUniqueSenders].
...
// nor regular packets can starve the other. It returns the first write
// error.
func (c *sclient) writePending(pend writerPending) (frames int, err error) {
	...
	for pend&(pendSendQueue|pendDiscoQueue) != 0 {
